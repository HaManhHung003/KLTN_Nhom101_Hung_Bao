...
const methodLabels = { vnpay: 'VNPay', momo: 'MoMo', bank: 'Chuyển khoản' }
const statusConfig = {
...
export function PaymentsView({ role }: PaymentsViewProps) {
  const titles = {
    buyer: { title: 'Đặt cọc & Thanh toán', desc: 'Lịch sử giao dịch, biên lai điện tử — FR-P01~P03' },
    ...
    <div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-900">{transactions.filter((x) => x.status === 'completed').length}</p>
          <p className="text-sm text-slate-500">Giao dịch thành công</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <CreditCard className="h-6 w-6 text-amber-600" />
          <p className="mt-2 text-2xl font-bold text-slate-900">{transactions.filter((x) => x.status === 'pending').length}</p>
          <p className="text-sm text-slate-500">Đang chờ xử lý</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <AlertCircle className="h-6 w-6 text-orange-600" />
          <p className="mt-2 text-2xl font-bold text-slate-900">{transactions.filter((x) => x.status === 'disputed').length}</p>
            ...
            <div key={tx.id} className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center">
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 flex flex-wrap gap-4 text-sm">
                  ...
                  <span className="text-slate-500">{methodLabels[tx.method]}</span>
